...
│  └─ src/
│     ├─ api/               admin-api 호출 (client, auth, applications, members)
...
- 모노레포 골격 + **1차 단계(로그인 · 앱 선택 · 회원) 구현 완료**.
- admin-api
  ...
  - `./gradlew build` 성공 (Testcontainers로 실제 Postgres에 V1~V3 적용 + 로그인/잠금/앱 목록/회원 프록시+감사로그 흐름까지 통합 테스트로 검증).
- admin-web
  - react-router-dom 재설치(로그인/회원 페이지가 생기면서 라우팅 필요해짐). `AuthContext`가 `/auth/me`로 세션 확인, `RequireAuth`가 미인증 시 `/login`으로 리다이렉트.
  - `LoginPage`, `AppLayout`(앱 선택기가 `GET /applications` 조회로 대체됨), `MembersPage`(검색·페이지네이션·상세 Drawer·정지 Modal·강제 로그아웃 Popconfirm).
  - `npm run build`, `npm run lint` 통과.
- infra: docker-compose(postgres 16 + admin-api + nginx) 빌드·기동 확인. `.env.example`에 `ADMIN_BOOTSTRAP_*` 추가.
- **실제 앱(꼬리) admin API가 아직 없어** 회원 목록/상세/정지/강제 로그아웃은 로컬에서 502(연동 실패)로 응답하는 것까지만 확인했다. 그 외 로그인/잠금/앱 선택은 docker compose로 기동해 curl로 전체 플로우(로그인 → 세션 쿠키 → CSRF → 잠금 → 앱 목록)를 end-to-end로 검증했다.
